...
package org.sterl.education.strongid;
 ...
 * 
 * TODO verify with the new hibernate 5 version, this code was written for hibernate 4
 *
 * @author sterlp
 */
public class UsingListsTest {
    // Entities which have a uni relation -- not biderectional
    @Entity(name = "PersonBE")
    @Table(name = "PERSON")
    @Data
    @NoArgsConstructor
    ...
    @Entity(name = "ItemBE")
    @Table(name = "ITEM_BE") // otherwise a reserved keyword
    @Data @ToString(of = {"id", "name"})
    ...
    @Entity(name = "OrderBE")
    @Table(name = "ORDER_BE") // ORDER is a reserved keyword in H2 DB
    @Data
    @NoArgsConstructor
    ...
    public void before() {
        ...
        session.createQuery("DELETE FROM ItemBE").executeUpdate();
        session.createQuery("DELETE FROM OrderBE").executeUpdate();
        session.createQuery("DELETE FROM PersonBE").executeUpdate();
        session.createQuery("DELETE FROM OfficeBE").executeUpdate();
