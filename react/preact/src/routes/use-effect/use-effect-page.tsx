...
class Service {
  constructor(private httpService: any) {
    console.info("[Service] constructor called ...");
  }
  doSomething() {
...
function UseEffectPage() {
  ...
  return (
    <>
      <ProductList httpService={httpService} />
    </>
  );
}
...
function ProductList({ httpService }: ProductListProps) {
  console.info("[ProductList] rendered ...");
  let useEffectRun = false;
  const [products, setProducts] = useState<any[]>([]);
  useEffect(() => {
    useEffectRun = true;
    setProducts(fetchProducts().products);
    console.info("ProductList useEffect");
  }, [httpService]);
  return (
    <>
      <Button onClick={() => setProducts(fetchProducts().products)}>
        Reload
      </Button>
      <ol>
        {products.map((p) => (
          <ProductListItem key={p.id} p={p} httpService={httpService} />
        ))}
      </ol>
    </>
  );
